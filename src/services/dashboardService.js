...
//         .catch(() => ({ data: { total: 0 } })), // 👈 6. POSICIÓN CORRECTA
...
//       api.get("/gastos/count").catch(() => ({ data: { total: 0 } })),
//       api.get("/logs/count").catch(() => ({ data: { total: 0 } })),
//     ]);

//     console.log("BAJO STOCK---------->", bajoStock.data.total);
...
//       productosBajoStock: bajoStock.data.total, // 👈 Ahora sí tiene el valor de la consulta 6
