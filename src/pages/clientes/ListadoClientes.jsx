...
const ListadoClientes = () => {
  ...
  // --- LÓGICA: CARGAR SALDO BILLETERA VIRTUAL ---
  const handleCargarBilletera = async (cliente) => {
    ...
    if (formValues && formValues[0] > 0) {
      try {
        ...
        Swal.fire(
          "¡Éxito!",
          "Saldo cargado en la billetera virtual.",
          "success"
        );
        fetchClientes(); // Recargar para ver el nuevo saldo
      } catch (error) {
              ...
              <tbody>
                {clientes.map((c, index) => (
                  <tr key={c.id}>
                    ...
                    <td className="text-right align-middle text-primary">
                      ${" "}
                      {parseFloat(c.pagos || 0).toLocaleString("es-AR", {
                        minimumFractionDigits: 2,
                      })}
                    ...
                    <td className="text-center align-middle">
                      <div className="btn-group shadow-sm">
                        ...
                        {c.id !== 1 && (
                          <>
                            {/* BOTÓN CARGAR BILLETERA */}
                            <button
                              className="btn btn-success btn-sm"
                              data-toggle="tooltip"
                              title="Cargar Billetera Virtual"
