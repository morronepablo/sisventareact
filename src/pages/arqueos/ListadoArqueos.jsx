...
const ListadoArqueos = () => {
  ...
  const formatDetails = (d) => {
    ...
    if (d.movimientos && d.movimientos.length > 0) {
      ...
      html +=
        '<thead class="thead-dark text-center" style="font-size:0.75rem"><tr><th>Tipo</th><th>Monto</th><th>Descripción</th></tr></thead><tbody>';
      d.movimientos.forEach((m) => {
        html += `<tr><td class="text-center"><span class="${m.tipo === "Ingreso" ? "text-success" : "text-danger"}"><b>${m.tipo}</b></span></td><td class="text-right">${formatMoney(m.monto)}</td><td>${m.descripcion || ""}</td></tr>`;
      });
      html += "</tbody></table>";
    } else {
      html +=
    ...
    if (parseFloat(d.total_retiros) > 0) {
      html += `<div class="alert alert-warning py-1 px-2 text-xs mb-0 mt-2 shadow-sm"><i class="fas fa-exclamation-triangle mr-1"></i> Retiros de seguridad: <b>${formatMoney(d.total_retiros)}</b></div>`;
    }
    html += "</div>";
    return html;
  };
  ...
  useEffect(() => {
    if (!loading && arqueos.length > 0) {
      ...
      const timer = setTimeout(() => {
        ...
        const initTooltips = () => {
          $('[data-toggle="tooltip"]')
            .tooltip({
              boundary: "window",
              trigger: "hover", // 👈 FIX 1: Solo mostrar al pasar el mouse, no al hacer foco/clic
            })
            .on("click", function () {
              $(this).tooltip("hide"); // 👈 FIX 2: Forzar ocultamiento inmediato al hacer clic
            });
        };

        // 🟢 FIX TOOLTIPS: Inicializar en cada "draw" (dibujo) de la tabla
      ...
      return () => {
        clearTimeout(timer);
        if (window.$ && window.$.fn.DataTable.isDataTable(tableId)) {
          $('[data-toggle="tooltip"]').tooltip("dispose"); // Limpieza de memoria
          $(tableId).DataTable().destroy();
        }
      };
    }
          ...
          <div className="card-header">
            ...
            <div className="card-tools">
              <button
                ...
                onClick={() =>
                  window.open(
                    `${API_URL}/api/arqueos/reporte?token=${localStorage.getItem("token")}`,
                    "_blank",
                  )
                }
              >
                ...
                {arqueos.map((arq, index) => {
                  ...
                  return (
                    <tr key={arq.id} data-index={index}>
                      ...
                      <td className="text-right font-weight-bold">
                        {isClosed ? formatMoney(arq.monto_final) : "–"}
                      </td>
                      <td className="text-right">
                        {formatMoney(arq.ventas_efectivo)}
                      </td>
                      ...
                      {isAdmin && (
                        <td style={{ minWidth: "210px" }}>
                          ...
                          >
                            <div
                              className="col-3 p-0 text-success"
                              data-toggle="tooltip"
                              title="Ventas + Entradas"
                            >
                              <b>Ing.</b>
                              <br />
                              {formatMoney(v_efe + ing)}
                            ...
                            <div
                              className="col-3 p-0 text-danger"
                              data-toggle="tooltip"
                            ...
                            <div
                              className="col-3 p-0 text-maroon"
                              data-toggle="tooltip"
                              title="Retiros Seguridad"
                            >
                              <b>Ret.</b>
                              <br />
                              {formatMoney(ret)}
                            ...
                            <div
                              className={`col-3 p-0 ${isClosed && Math.abs(valorAMostrar) > 1 ? "text-danger text-bold" : "text-primary text-bold"}`}
                              data-toggle="tooltip"
                              title={
                                isClosed
